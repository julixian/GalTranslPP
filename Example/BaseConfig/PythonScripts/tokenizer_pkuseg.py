...
def _model_probe_worker(model_name: str, result_queue) -> None:
    try:
        ...
        pkuseg.pkuseg(model_name=model_name, postag=True)
...
def ensure_model(model_name: str) -> bool:
    ...
    script = (
        "import pkuseg; "
        f"pkuseg.pkuseg(model_name={model_name!r}, postag=True)"
    )
...
class NLPProcessor:
    """
    pkuseg 分词代理。父解释器只保存轻量对象，pkuseg 模型在独立子进程中加载。
    """
    def __init__(self, model_name: str):
        ...
        if message_type == "init_error":
            raise RuntimeError(
                f"pkuseg worker failed to load model {model_name}: "
                f"{message.get('errorType')}: {message.get('error')}\n{message.get('traceback', '')}"
            )
        raise RuntimeError(f"pkuseg worker returned unexpected init message: {message!r}")

    def process_text(self, text: str):
        if self._closed:
            raise RuntimeError("pkuseg worker is already closed")
        ...
        while True:
            try:
                ...
                raise TimeoutError("Timed out waiting for pkuseg worker result") from e
            ...
            if message_type == "error":
                raise RuntimeError(
                    f"pkuseg worker failed: {message.get('errorType')}: {message.get('error')}\n"
                    f"{message.get('traceback', '')}"
                )
            raise RuntimeError(f"pkuseg worker returned unexpected message: {message!r}")
