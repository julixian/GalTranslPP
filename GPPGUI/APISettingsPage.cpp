...
void APISettingsPage::setupUi()
{
    ...
    ElaDoubleText* apiStrategyTitle = new ElaDoubleText(tr("API 使用策略"), 16,
        tr("令牌策略，random随机轮询，fallback优先第一个，出现[请求错误]时使用下一个"), 10,
        "", apiStrategyArea);
...
ElaScrollPageArea* APISettingsPage::createApiInputRowWidget(const toml::value& api)
{
    ...
    ElaDoubleText* temperatureConfigTitle = new ElaDoubleText(tr("温度"), 16,
    ...
    ElaDoubleText* topPConfigTitle = new ElaDoubleText(tr("top_p"), 16,
    ...
    ElaDoubleText* frequencyPenaltyConfigTitle = new ElaDoubleText(tr("frequency_penalty"), 16,
    ...
    ElaDoubleText* presencePenaltyConfigTitle = new ElaDoubleText(tr("presence_penalty"), 16,
    ...
    ElaDoubleText* extraKeysTitle = new ElaDoubleText("Extra keys", 18,
        "", 0, tr("一行一个 key"), configWidget);
