...
public:
	...
	static QList<GuiGptDictEntry> readGptDict(const fs::path& dictPath);
	static QList<GuiGptDictEntry> readGptDicts(const std::vector<fs::path>& dictPaths);

	static QList<GuiNormalDictEntry> readNormalDict(const fs::path& dictPath);
