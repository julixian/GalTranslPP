...
- 页面右上角的“刷新”图标和公开方法 `refreshCacheFiles()` 会调用 `_loadCacheFiles(true)`，用于放弃未保存的缓存编辑并按磁盘状态重新加载。
...
- 页面首次进入或删除文件后的内部刷新会保留脏文件的内存副本，避免其它未保存编辑被连带丢弃。
- 用户主动点击右上角“刷新”或外部调用 `refreshCacheFiles()` 时会清空 `_dirtyFiles` 和 `_loadedEntriesByFile`，也就是明确放弃未保存修改并重读磁盘。
- 如果当前文件仍存在，会重新加载当前文件；主动刷新时当前文件也以磁盘内容为准，文件名前的 `*` 会消失。
