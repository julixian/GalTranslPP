            ...
            return runLazyTokenizer(state, text, [&]() -> std::function<NLPResult(const std::string&)>
                {
                    ...
                    if (!mecabModel) {
                        throw std::runtime_error(gppTr(
                            ...
                            "无法初始化 MeCab Model。请确保 BaseConfig/mecabDict/mecabrc 和 %1 存在\n错误信息: %2")
                            .arg(mecabDictDir)
                    ...
                    if (!mecabTagger) {
                        throw std::runtime_error(gppTr(
                            "NLPTool.getMeCabTokenizeFunc",
                            "无法初始化 MeCab Tagger。请确保 BaseConfig/mecabDict/mecabrc 和 %1 存在\n错误信息: %2")
                            .arg(mecabDictDir)
                            .arg(MeCab::getLastError())
                            .toStdString());
                    }
            ...
            return runLazyTokenizer(state, text, [&]() -> std::function<NLPResult(const std::string&)>
                {
                    ...
                    if (initNeedReboot) {
                        throw std::runtime_error(gppTr(
                            "NLPTool.getNLPTokenizeFunc",
                            "需要重启程序以应用新安装的 NLP 模型")
                            .toStdString());
                    }
