...
absl::flat_hash_map<std::string, std::string> NameTranslator::translateBatch(std::span<const std::string> batchNames, int threadId) {
    ...
    while (retryCount == 0 || retryCount < m_maxRetries) {
        ...
        if (!checkResponse(
            response, m_apiPool, currentApi, ascii2Wide(gppTr("NameTranslator.translateBatch", "人名表翻译")), m_apiStrategy, m_controller, m_logger, retryCount, threadId, m_checkQuota
        )) {
            continue;
        }
    ...
    m_logger->error(gppTr("NameTranslator.translateBatch", "[线程 %1] NameTrans: 批次翻译失败，已达到最大重试次数。", threadId));
    return resultMap;
}

void NameTranslator::run(const fs::path& nameTablePath) {
    if (!fs::exists(nameTablePath)) {
        m_logger->error(gppTr("NameTranslator.run", "NameTrans: 未找到人名表文件 %1", wide2Ascii(nameTablePath)));
        return;
    }

    m_logger->info(gppTr("NameTranslator.run", "NameTrans: 开始处理人名表..."));
    ...
    try {
        ...
        m_logger->error(gppTr("NameTranslator.run", "NameTrans: 解析人名表失败: %1", e.what()));
        return;
    }
    ...
    m_logger->info(gppTr("NameTranslator.run", "NameTrans: 共发现 %1 个待翻译的名字。", namesToTranslate.size()));
    ...
    m_logger->info(gppTr("NameTranslator.run", "NameTrans: 启动 %1 个线程，每批处理 %2 个名字。", workerCount, m_batchSize));
    ...
    m_logger->info(gppTr("NameTranslator.run", "NameTrans: 处理完成，已更新 %1 个译名，保存至 %2", updatedCount, wide2Ascii(nameTablePath)));
}
