...
本项目在继承了 GalTransl 基本功能的基础上，包括但不限于对以下模块进行了优化：
...
对于熟悉 GalTransl 的人来说，过渡到 GalTransl++ CLI 版本可谓易如反掌，所以接下来主要讲 GUI。

这里还是再稍微谈一下翻译流程吧 (默认你已经读过 GalTransl 的使用说明，翻译接口什么的基础概念就略过了)。

GalTransl++ 目前无论处理哪种文件格式，最后都是统一化为 json 来读取。

每个 json 文件是一个对象列表，每个对象代表一个『句子』，这是喂给AI进行翻译的基本单位。

读取 json 时主要关注两个键，分别为 `name` (有时是`names`) 和 `message`，不同的翻译模式 (`transEngine`) 就是对这两个键进行各种不同的处理：


#### 翻译模式 (transEngine)

* **`# ForGalTsv`**: 实际翻译模式，向 AI 输入 TSV 格式的句子 (包含 `name` 和`message`) 并要求 AI 以 TSV 格式回复，程序将解析返回的 TSV，可能比 ForGalJson 模式更省 token 但破限效果可能更弱。
* **`# ForNovelTsv`**: 实际翻译模式，和 `ForGalTsv` 的区别主要是变动提示词，向 AI 输入和解析的时候都不带 `name` 键。
* **`# ForGalJson`**: 实际翻译模式，向 AI 输入 json 格式的句子 (包含 `name` 和 `message` ) 并要求 AI 以 json 格式回复，程序将解析返回的 JSON。
...
在 `Rebuild` 中所提到的缓存，是指翻译过后留存在项目文件夹下，`transl_cache` 文件夹中的 json 文件，其中按顺序存储了每个文件对应的序列号，原文和译文等信息。所有的实际翻译模式都会先读取缓存，然后只挑选出缓存中还没有的原文进行翻译。

> **⚠️ 特别注意**：在使用单文件分割功能的情况下，由于缓存命中结合了上下文，所以当你改变文件本身，或者 分割数/分割方式 时，会有一部分无关的句子不能命中缓存。理论上文件切的越碎，最终分割出的文件份数比最大线程数超过的更多，则不能命中缓存的句子越多。GalTransl++ 会尽可能在这种情况下保证原有缓存的命中 (你也可以调整 `分割缓存查找距离` 来控制缓存查找)，不过如果希望达到更好的缓存命中，最好还是不改变分割方式和分割数。为此也可以使用 `ShowNormal` 模式观察切割后的文件。
...
* **文件对应关系**
  ...
  * GUI 会读取项目文件夹下 `NameTable.toml` 来作为 **人名表** 中的数据。
...
* `index`: 索引顺序。 条件对象指代名 `index`
* `name`: 人名，展示的是预处理后的人名 (相当于 `pre_processed_name`)。 条件对象指代名 `name`
* `names`: 复数人名，多见于部分 VNT 提出的文本中。 条件对象指代名 `names`
...
* `pre_processed_text`: 经过一系列预处理后，即将执行 AI 翻译前时句子的样子。 条件对象指代名 `preproc`
* `other_info`: 其它附加信息。 条件对象指代名 `otherinfo`
* `problems`: 在自动化找错中找出的问题。 条件对象指代名 `problems`
* `translated_by`: 翻译此句子所用的模型及 apikey。 条件对象指代名 `transby`
* `translated_raw_text`: AI 返回的，未经过任何后处理时句子的样子。 条件对象指代名 `transraw`
* `translated_view_text`: 经过所有后处理之后，将输出的 message。 条件对象指代名 `transview`
* `_gpp_ref_to`: 开启连续重复块引用复用时的引用信息，指明程序不会翻译该句而是会复用此键所指向句子的翻译
* `_gpp_ref_by`: 引用信息，指明该句被哪些句子引用
* `_gpp_ref_pending`: 引用信息，指明此句是否仍在等待 `_gpp_ref_to` 所指向句子的翻译

### 替换型字典语法

* **译前字典** 搜索并替换 `pre_processed_text`，时机详见 [处理与翻译顺序](#%EF%B8%8F-处理与翻译顺序)。
* **译后字典** 搜索并替换 `translated_view_text`，时机详见 [处理与翻译顺序](#%EF%B8%8F-处理与翻译顺序)。
...
conditions = [
  ...
  {conditionReg='母親', conditionTarget='prev_preproc'},
]
```

同一 `conditions` 中的所有条件为隐式 AND，必须全部命中才执行词条。需要 OR 时请拆成多个词条；需要 NOT 时请直接在 `conditionReg` 中使用否定正则。

`conditionTarget` 可使用 `index`, `filename`, `name`, `names`, `nametrans`, `namestrans`, `orig`, `preproc`, `problems`, `otherinfo`, `transby`, `transraw`, `transview`。目标前可重复添加 `prev_` 或 `next_` 以检查相邻句，例如 `prev_orig` 和 `prev_prev_preproc`。

出于常用性和 GUI 支持等方面的考量，条件字典不像 `retranslKeys`/`skipProblems` 那样可以外接脚本，如果有复杂的替换条件建议直接写一个自定义文本插件。

当 `isReg = true` 时，`org` 和 `rep` 将作为正则搜索与替换表达式。

优先级越高的词条越先执行。

## ⚙️ 处理与翻译顺序

1、  可选: 执行 dprerun 阶段的插件。  
2、  可选: 对 `name` 执行译前字典替换 (实际上在最开始收集人名表的时候也会执行一次以收集替换后的 `name`，只不过使用的是临时的句子结构体，无缓存信息)。  
3、  将 `message` 中的换行统一替换为 `<br>`。  
4、  将 `message` 中的制表符统一替换为 `<tab>`。  
5、  可选: 对 `message` 执行译前字典替换。  
6、  可选: 执行 prerun 阶段的插件。  
7、  **AI 翻译**。  
8、  解析 **翻译失败 (Failed to translate)** 和 **GPPCProblem**，如果翻译失败则不分析其它问题。  
9、  可选: 执行 postrun 阶段的插件。  
10、  可选: 对 `message` 执行译后字典替换。  
11、  将 `message` 中的 `<tab>` 统一替换回制表符。  
12、  将 `message` 中的换行从 `<br>` 换回原符号。  
13、  可选: 对 `name` 执行GPT字典替换 (此时 GPT字典 将被临时视为 **替换型字典**)。  
14、  可选: 对 `name` 执行人名表替换 (人名表译名为空则忽略)。  
15、  可选: 对 `name` 执行译后字典替换。  
16、  可选: 执行 dpostrun 阶段的插件。  
17、  可选: 问题分析。  
...
> **正则引擎补充**
>
> 虽然 GalTransl++ 的核心为 C++ 编写，但由于正则引擎采用 pcre2 且默认使用 unicode 参数编译正则，所以可自定义编辑的所有正则依然默认是以字符为单位的。比如即使 emoji 表情'😪' 在 UTF-8 中占用 4 个字节，在正则中依然是以一个 `.` 来匹配的。
>
> 同时也支持 pcre2 所支持的所有扩展正则语法，如可以使用 `\p{P}` 来匹配任意标点符号，`\p{Hangul}` 来匹配任意韩文字符等。
...
# 也可以指定 conditionScript 和 conditionFunc 来外接 lua/py 脚本 (conditionFunc 接收 Sentence，返回 bool)
...
# (conditionFunc 接收 Sentence 和 problem，返回 bool)
skipProblems = [
  #"^引入拉丁字母: Live$"  # 不加任何条件
...
`base` 和 `check` 可使用 `orig`, `preproc`, `transraw`, `transview`。
</details>
...
GalTransl++ 将仅使用 [GUMBO](https://codeberg.org/gumbo-parser/gumbo-parser) 库遍历 Epub 中的 HTML/XHTML 文件
(Epub 文件本身只是一个 zip 包，你可以使用任何解压软件打开它并浏览其中的文件)，
...
```
则可在理想位置还原振假名效果 (可以修改提示词告诉 AI 注音格式 `[振假名/基本文本]` 及保留要求来达到更好的保留效果)。当然如果只想在原文中保留振假名而在译文中删去，那也很简单，直接在译前字典中再写一个去注音字典即可。其它很多自定义需求也都可借此满足。
...
```
假如我不想一个一个文件的看标签写正则，而是想要直接『删除所有`<p></p>`标签内的其余标签并保留非标签部分』来快速过滤的话，面对不擅长处理嵌套的简单正则，显然我们很难写出这样的 正则/正则组 来处理这个问题，那么此时就需要使用回调正则。
...
- 2、 访问 [PyTorch 官网](https://pytorch.org/get-started/locally/)，选择合适的 CUDA 版本获取安装命令。
...
由于我自己的开发和构建环境基本绑定 Windows 系统，我自己也没有 Linux/Mac 设备，所以即使在代码本身中使用的 WinApi 数量很少也很好替换，
